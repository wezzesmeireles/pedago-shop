...
function buildHtml(o: { title: string; desc: string; image: string; url: string; storeName: string }) {
...
export default async function handler(req: any, res: any) {
  ...
  const [cfgRows, productRows] = await Promise.all([
    ...
    slug
      ? supaGet(`products?slug=eq.${encodeURIComponent(slug)}&is_active=eq.true&deleted_at=is.null&select=name,description,cover_image_url,price,slug&limit=1`).catch(() => [])
      : Promise.resolve([]),
  ]);
  ...
  res.status(200).send(buildHtml({ title, desc, image, url, storeName }));
}
