  ...
  <div class="fixed inset-0 z-50 flex items-end md:items-stretch md:justify-end" @click.self="$emit('close')">
    ...
    <div class="relative bg-[#f8f7fc] w-full md:max-w-sm h-[90svh] md:h-full flex flex-col shadow-2xl rounded-t-3xl md:rounded-none animate-slide-in-bottom md:animate-slide-in-right">
      ...
      <div class="flex-1 overflow-y-auto py-4 px-4 space-y-2.5">
          ...
          >
            ...
            <div class="flex-1 min-w-0 flex flex-col py-0.5">
              <div class="flex justify-between items-start gap-2">
                <RouterLink :to="`/produto/${item.slug}`" @click="$emit('close')" class="flex-1">
                ...
                <button
                  ...
                  class="flex-shrink-0 w-7 h-7 flex items-center justify-center text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all duration-150 active:scale-90"
              ...
              <div class="flex items-end justify-between mt-auto pt-2">
                <div class="flex flex-col">
                  <p class="text-base font-black text-violet-600 leading-none">{{ formatPrice(item.price * item.quantity) }}</p>
                  <span v-if="item.quantity > 1" class="text-[10px] text-gray-400 mt-1 font-medium">{{ item.quantity }}x {{ formatPrice(item.price) }}</span>
                  <span v-else class="text-[11px] text-green-600 font-semibold mt-1">⚡ Imediato</span>
                </div>

                <!-- Controle de Quantidade -->
                <div class="flex items-center bg-gray-50 rounded-lg border border-gray-100 p-0.5 shadow-sm">
                  <button @click="cart.updateQuantity(item.productId, item.quantity - 1)" class="w-7 h-7 flex items-center justify-center text-gray-500 hover:text-violet-600 hover:bg-white rounded-md transition-colors active:scale-95 shadow-sm" :aria-label="`Diminuir quantidade de ${item.name}`">
                  ...
                  <span class="w-7 text-center text-xs font-bold text-gray-700 select-none">{{ item.quantity }}</span>
                  <button @click="cart.updateQuantity(item.productId, item.quantity + 1)" class="w-7 h-7 flex items-center justify-center text-gray-500 hover:text-violet-600 hover:bg-white rounded-md transition-colors active:scale-95 shadow-sm" :aria-label="`Aumentar quantidade de ${item.name}`">
        ...
        <!-- Cross-sell (Compre Também) -->
        <div v-if="crossSellProducts.length > 0 && cart.items.length > 0" class="pt-4 mt-4 border-t border-gray-100">
          <h4 class="text-[11px] font-black text-gray-400 uppercase tracking-wider mb-3 px-1">Compre também</h4>
          <div class="space-y-2">
            <div
              v-for="product in crossSellProducts"
              :key="product.$id"
              class="flex items-center gap-3 bg-white border border-gray-100 rounded-xl p-2 shadow-sm hover:border-violet-100 transition-colors"
            >
              <RouterLink :to="`/produto/${product.slug}`" @click="$emit('close')" class="w-12 h-12 rounded-lg bg-gray-100 flex-shrink-0 overflow-hidden">
                <img v-if="product.coverImageUrl" :src="product.coverImageUrl" :alt="product.name" class="w-full h-full object-cover hover:scale-105 transition-transform" />
              </RouterLink>
              <div class="flex-1 min-w-0 flex flex-col justify-center">
                <RouterLink :to="`/produto/${product.slug}`" @click="$emit('close')">
                  <p class="text-xs font-bold text-gray-800 line-clamp-1 leading-snug hover:text-violet-600 transition-colors">{{ product.name }}</p>
                </RouterLink>
                <p class="text-xs font-black text-violet-600 mt-0.5">{{ formatPrice(product.price) }}</p>
              </div>
              <button
                @click="cart.add({ productId: product.$id, name: product.name, price: product.price, coverImageUrl: product.coverImageUrl || '', slug: product.slug })"
                class="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-violet-50 text-violet-600 hover:bg-violet-600 hover:text-white rounded-lg transition-colors active:scale-95"
...
onMounted(() => {
  if (catalog.featuredProducts.length === 0) {
    catalog.fetchFeatured();
  }
});

const crossSellProducts = computed(() => {
  return catalog.featuredProducts.filter((p) => !cart.items.some(i => i.productId === p.$id)).slice(0, 3);
});
