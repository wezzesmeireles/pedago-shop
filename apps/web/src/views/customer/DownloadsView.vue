<template>
  <div>
    ...
    <transition name="fade">
      <div v-if="downloadNotice" class="mb-5 flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl p-4" role="status">
        <span class="text-sm text-blue-700">{{ downloadNotice }}</span>
      </div>
    </transition>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-2">
              ...
              <div class="flex-shrink-0">
                ...
                <!-- Link delivery -->
                <a v-else-if="download.deliveryLink" :href="download.deliveryLink" target="_blank" rel="noopener" @click="guardExternalLink"
                ...
                <button v-else type="button" @click="downloadFile(download)" :disabled="preparingToken === download.token"
                  class="inline-flex items-center gap-1.5 bg-primary-600 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-primary-700 disabled:opacity-60 disabled:cursor-wait transition-colors whitespace-nowrap">
                  <svg v-if="preparingToken === download.token" class="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none">
                  ...
                  <span>{{ downloadButtonLabel(download.token) }}</span>
                </button>
              </div>
            </div>
...
async function downloadFile(d: DownloadEntry) {
  ...
  try {
    if (supportsIosFileSave()) {
      const prepared = preparedFiles.value[d.token];
      if (prepared) {
        await savePreparedFile(prepared);
        return;
      }

      preparingToken.value = d.token;
      const file = await prepareTokenDownload(d.token, d.productName);
      preparedFiles.value = { ...preparedFiles.value, [d.token]: file };
...
function downloadButtonLabel(token: string): string {
  ...
  return supportsIosFileSave() ? 'Preparar PDF' : 'Baixar';
}
