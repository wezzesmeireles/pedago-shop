            ...
            <li class="flex gap-3">
              ...
              <div>
                ...
                <ul class="mt-2 space-y-1 ml-1">
                  ...
                  <li class="text-xs text-gray-600">• URL: <code class="bg-white px-1 rounded">{{ apiUrl }}/webhooks/mercadopago</code></li>
          ...
          <ol class="space-y-3 text-sm text-gray-700">
            ...
            <li class="flex gap-3">
              ...
              <div>
                ...
                <code class="block bg-white border border-gray-200 rounded-lg px-3 py-1.5 mt-1 text-xs">{{ googleCallbackPreview }}</code>
              </div>
            </li>
            <li class="flex gap-3">
              <span class="w-5 h-5 bg-indigo-100 text-indigo-700 rounded-full text-xs flex items-center justify-center font-bold flex-shrink-0 mt-0.5">5</span>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="bg-blue-50 rounded-xl p-3 flex gap-2 text-xs text-blue-700">
            ...
            <span>Configure o webhook no painel do Mercado Pago apontando para: <code class="bg-blue-100 px-1 rounded">{{ apiUrl }}/webhooks/mercadopago</code></span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <div class="ml-auto">
            <span :class="['text-xs px-2.5 py-1 rounded-full font-medium', form.googleClientId ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500']">
              {{ form.googleClientId ? 'Configurado' : 'Não configurado' }}
            </span>
          </div>
        </div>

        <div class="p-6 space-y-4">
          <div class="bg-amber-50 rounded-xl p-3 flex gap-2 text-xs text-amber-700 mb-2">
            ...
            <span>Alterações no Google OAuth requerem reinício do servidor para entrar em vigor.</span>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent font-mono"
            />
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <input
                ...
                class="w-full border border-gray-300 rounded-xl px-4 py-2.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent font-mono"
              />
              <button @click="showGoogleSecret = !showGoogleSecret" type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">URL de Callback</label>
            <input
              ...
              class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent font-mono"
            />
            <p class="mt-1 text-xs text-gray-400">Adicione esta URL como "URI de redirecionamento autorizado" no Google Cloud Console.</p>
          </div>
        </div>
      </div>
...
const googleCallbackPreview = computed(() =>
  form.value.googleCallbackUrl || 'http://localhost:3000/auth/google/callback'
);
...
async function save() {
  ...
  savedAt.value = false;
  try {
    await api.patch('/admin/integrations', form.value);
    savedAt.value = true;
    setTimeout(() => (savedAt.value = false), 3000);
  } catch (e: any) {
    error.value = e?.response?.data?.message || 'Erro ao salvar. Tente novamente.';
  } finally {
    saving.value = false;
  }
}
