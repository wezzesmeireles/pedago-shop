<template>
  <div class="max-w-3xl">
    ...
    <div v-else class="space-y-4">
      ...
      <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3 flex-wrap gap-y-2">
          <div class="w-10 h-10 bg-[#5865F2]/10 rounded-xl flex items-center justify-center flex-shrink-0 text-[#5865F2]">
          ...
          <div>
            <h2 class="font-semibold text-slate-900">Discord Webhook</h2>
            <p class="text-xs text-slate-500">Notificações automáticas de vendas em tempo real para a equipe</p>
          </div>
          <div class="ml-auto">
            <span :class="['text-xs px-2.5 py-1 rounded-full font-semibold', form.discordWebhookUrl ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500']">
              {{ form.discordWebhookUrl ? 'Conectado' : 'Não configurado' }}
            </span>
          </div>
        </div>
        <div class="p-4 sm:p-6 space-y-4">
          <p class="text-xs text-slate-500">
            Toda venda aprovada ou pedido gerado será enviado imediatamente para o canal do Discord vinculado a este webhook. Toda a equipe que estiver no canal receberá a notificação com som no celular.
          </p>

          <div>
            <label class="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">URL do Webhook do Discord</label>
            <div class="flex items-center gap-2">
              <input v-model="form.discordWebhookUrl" type="text"
                placeholder="https://discord.com/api/webhooks/..."
                class="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#5865F2] focus:border-transparent font-mono" />
              <button @click="testDiscord" :disabled="!form.discordWebhookUrl || testingDiscord" type="button"
                class="flex-shrink-0 inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-[#5865F2]/40 text-[#5865F2] hover:bg-[#5865F2]/10 font-bold text-xs transition-all disabled:opacity-40">
                <svg v-if="testingDiscord" class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>
                <span>{{ testingDiscord ? 'Enviando...' : 'Testar no Discord' }}</span>
              </button>
            </div>
            <Transition name="fade-save">
              <span v-if="discordTestResult" :class="['inline-block mt-2 text-xs font-bold px-2 py-0.5 rounded-md', discordTestResult === 'ok' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600']">
                {{ discordTestResult === 'ok' ? '✓ Mensagem de teste enviada com sucesso para o canal!' : '✗ Erro ao disparar webhook. Verifique o link.' }}
              </span>
            </Transition>
          </div>
        </div>
...
async function testDiscord() {
  if (!form.value.discordWebhookUrl) return;
  testingDiscord.value = true;
  discordTestResult.value = '';
  try {
    const res = await fetch(form.value.discordWebhookUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        content: '🎉 **Teste de Notificação de Vendas (Discord)!**\nSeu canal está configurado com sucesso.',
        embeds: [{
          title: '✅ Teste de Conexão',
          color: 0x5865F2,
          description: 'As notificações de compras aprovadas serão enviadas aqui em tempo real para toda a equipe.',
          footer: { text: 'Site Pedagógico' },
          timestamp: new Date().toISOString()
        }]
      })
    });
    discordTestResult.value = res.ok ? 'ok' : 'erro';
  } catch {
    discordTestResult.value = 'erro';
  } finally {
    testingDiscord.value = false;
    setTimeout(() => (discordTestResult.value = ''), 4000);
  }
}
...
onMounted(async () => {
  try {
    ...
    form.value.discordWebhookUrl = v.discordWebhookUrl ?? '';
